                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            @component($prefix_component."navtab",['number'=>2,'titles'=>['لیست','جستجو']])
                                @slot("tabContent0")
                                    @if(isset($product[0]))
                                        @component($prefix_component."form",['action'=>route('admin.product.action_all')])
                                            @slot("content")
                                                <table class="table text-center">
                                                    ...
                                                    @foreach($product as $item)
                                                        <tr>
                                                            ...
                                                            <td>{{ $loop->iteration + $product->firstItem() - 1 }}
                                                            <td>{{$item["title"]}}</td>
                                                            <td>مدیر اصلی</td>
                                                            <td>{{$item->product_cat->title ?? ""}}</td>
                                ...
                                @slot("tabContent1")
                                    @component($prefix_component."form",['method'=>'get'])
                                        @slot("content")
                                            ...
                                            @component($prefix_component."select_recursive",['name'=>'catid','value'=>request()->get('catid'),'options'=>$product_cats_search,'label'=>'دسته بندی','choose'=>true, 'sub_method'=>'sub_cats'])@endcomponent
                                            <div class="my-3">
